...
def load_validation_results(project_root: Path) -> Dict[str, Dict[str, str]]:
    """
    ...
    # Search in multiple locations relative to project root
    search_dirs = [
        project_root / "validation",
        project_root / "results" / "validation",
        project_root / "benchmarks" / "validation",
    ]

    for search_dir in search_dirs:
        if not search_dir.exists():
            continue
        for file_path in search_dir.glob("*results.json"):
            ...
            try:
                ...
                validation_hash = data.get("validation_hash", data.get("hash", ""))
...
def validate_hashes(validation_results: Dict[str, Dict[str, str]]) -> Dict[str, Dict[str, Any]]:
    ...
    for scenario, lang_hashes in validation_results.items():
        ...
        lang_hashes = {k: v for k, v in lang_hashes.items() if v}
        if not lang_hashes:
            continue
...
def generate_validation_report(validation_report: Dict[str, Any], output_dir: Path):
    ...
    with open(report_file, 'w') as f:
        f.write('\n'.join(report_lines))
    print(f"  ✓ Saved thesis_validation_report.md")


def check_synthetic_data_consistency(project_root: Path) -> bool:
    """Check that synthetic data files exist and are consistent."""
    synthetic_dir = project_root / "data" / "synthetic"
    if not synthetic_dir.exists():
        print("Warning: Synthetic data directory not found")
        print("  Run: python3 tools/generate_synthetic_data.py")
        return False
...
def main():
    ...
    parser.add_argument("--input", "-i", type=Path, default=None,
                       help="Project root directory (containing results/ and validation/)")
    ...
    if args.output is None:
        args.output = project_root / "results" / "validation"

    print("=" * 60)
    print("RUNNING CROSS-LANGUAGE VALIDATION")
    print("=" * 60)
    print(f"Project root: {args.input}")
    print(f"Output directory: {args.output}")
    print()

    # Check synthetic data
    if args.check_data or args.all:
        check_synthetic_data_consistency(args.input)
        print()
    ...
    for scenario in sorted(validation_results.keys()):
